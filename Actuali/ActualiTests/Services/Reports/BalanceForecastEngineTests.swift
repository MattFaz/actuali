...
struct BalanceForecastEngineTests {
    ...
    @Test func transferScheduleProjectsBothLegs() throws {
        let schedules = try [schedule(nextDate: 20_260_520, payee: "p-transfer", amount: -25000, dateCondition: .fixed(#require(DayDate(yyyymmdd: 20_260_520))))]
    ...
    @Test func dailyBoundaryBetweenHistoryAndForecast() throws {
        let transactions = [tx(date: 20_260_512, amount: 1000)]
        let schedules = try [schedule(nextDate: 20_260_516, amount: -300, dateCondition: .fixed(#require(DayDate(yyyymmdd: 20_260_516))))]
        let result = BalanceForecastEngine.compute(
