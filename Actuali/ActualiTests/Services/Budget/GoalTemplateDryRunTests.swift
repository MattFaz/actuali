...
struct GoalTemplateDryRunTests {
    ...
    @Test func splitsScheduleBatchByMonthlyContribution() throws {
        ...
        let result = try dryRun(
            [
                ...
                parse("#template-1 schedule Water")
            ],
            schedules: [
    ...
    @Test func splitsRemainderByWeight() throws {
        ...
        let result = try dryRun([
            parse("#template remainder 2"),
            parse("#template remainder")
        ], sheet: sheet)
    ...
    @Test func validationErrorsReturnZeros() throws {
        let result = try dryRun([parse("#template-1 schedule Ghost")])
    ...
    @Test func clampedRunScalesContributions() throws {
        ...
        let context = try GoalTemplateContext(
            templates: [parse("#template-1 100"), parse("#template-1 200")],
            category: category,
            month: "2024-01",
            budgeted: 0,
    ...
    @Test func limitCapScalesContributions() throws {
        ...
        let context = try GoalTemplateContext(
            templates: [parse("#template-1 100 up to 150")],
            category: category,
            month: "2024-01",
            budgeted: 0,
            sheet: sheet,
