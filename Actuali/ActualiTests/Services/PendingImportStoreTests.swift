...
struct PendingImportStoreTests {
    ...
    @Test @MainActor func addAndRemove() throws {
    ...
    @Test @MainActor func removeAll() throws {
        let (store, url) = makeStore()
        defer { try? FileManager.default.removeItem(at: url) }
    ...
    @Test @MainActor func failedSaveDoesNotMutateTheQueue() throws {
    ...
    @Test @MainActor func visibleImportsKeepAllBudgetsVisibleForReview() throws {
        let (store, url) = makeStore()
        defer { try? FileManager.default.removeItem(at: url) }
