...
struct ScheduleLifecycleTests {
    ...
    @Test func ordinarySkipSearchesFromTheDayAfter() throws {
        let next = try #require(DayDate(yyyymmdd: 20_260_815)) // Saturday, no weekend solve
        let plain = config([:])
    ...
    @Test func skipStepsClearOfABeforeWeekendSolve() throws {
        let friday = try #require(DayDate(yyyymmdd: 20_260_814)) // Friday
    ...
    @Test func afterWeekendSolvingNeedsNoSpecialCase() throws {
        let friday = try #require(DayDate(yyyymmdd: 20_260_814))
    ...
    @Test func skippingAMonthlyScheduleLandsOnTheFollowingMonth() throws {
        let plain = config([:])
        let next = try #require(try ScheduleRecurrence.nextOccurrence(
            config: plain, onOrAfter: #require(DayDate(yyyymmdd: 20_260_816))
        ))
