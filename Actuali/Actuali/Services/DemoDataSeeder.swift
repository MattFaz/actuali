...
enum DemoDataSeeder {
    ...
    private static func insertSeedData(_ db: Database, tracking: Bool, now: Date) throws {
        ...
        try insertWidget(db, pageId: mainPageId, type: "markdown-card", y: 0, width: 12, height: 2, meta: """
            {"content":"**Welcome to the demo** 👋\\n\\nThis is sample data stored only on this device \u{2014} nothing you do here can touch a server or a real budget. When you\u{2019}re ready, connect your own Actual Budget server in **Settings → Connection & Data**."}
            """)
