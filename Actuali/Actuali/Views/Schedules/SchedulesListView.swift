...
struct SchedulesListView: View {
    ...
    var body: some View {
                    ...
                    Section {
                        ...
                        if completedCount > 0 && !showCompleted {
                            Text(completedCount == 1
                                ? String(format: String(localized: "%lld completed schedule hidden."), Int64(completedCount))
                                : String(format: String(localized: "%lld completed schedules hidden."), Int64(completedCount)))
                        }
                    }
                }
            }
        ...
        .confirmationDialog(
            pendingDelete.map { _ in String(localized: "Delete this schedule?") } ?? "",
            isPresented: Binding(
