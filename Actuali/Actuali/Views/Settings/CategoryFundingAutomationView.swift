...
struct CategoryFundingAutomationView: View {
    ...
    var body: some View {
            ...
            Section {
                Picker("Funding Source", selection: $configuration.fundingSource) {
                    ...
                    ForEach(fundingCategories, id: \.id) { category in
                        Text(category.name).tag(CategoryFundingSource.category(category.id))
                    }
                }
            } header: {
                Text("Funding")
        ...
        .onChange(of: configuration.accountId) { _, accountId in
            if accountId == nil { configuration.isEnabled = false }
        }
        .onChange(of: configuration) { _, _ in
            CategoryFundingAutomation.saveConfiguration(configuration, for: budgetStore.currentBudgetId)
        }
    }
}