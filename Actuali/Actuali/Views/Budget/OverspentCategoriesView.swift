               ...
               !budget.overspentCategories.isEmpty {
                List {
                    Section {
                        ...
                        Text(String(format: String(localized: "Categories with a negative balance in %@. Balances include overspending rolled over from earlier months. Use the action under a category to resolve it, or open the category to review its transactions."), MonthPicker.title(for: budget.month)))
                    }
                }
