...
/// so 1, ., 0 produces 1.0.
///
/// The keyboard toolbar also has +, −, ×, ÷ buttons: typing "12.50", tapping
/// +, then "6" shows "12.50 + 6.00" live, and collapses to a plain "18.50"
/// as soon as the field loses focus (Done, tapping elsewhere, or Save) - so
/// the bound text is always a plain decimal amount, never a dangling
/// expression.
struct AmountInputField: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UITextField {
        ...
        // and the decimal pad has no return/operator keys — without this
        // accessory bar there is no way to dismiss the keyboard or do quick
        // math from this field.
        let toolbar = UIToolbar(frame: CGRect(x: 0, y: 0, width: 100, height: 44))
        var items: [UIBarButtonItem] = [
            UIBarButtonItem(
                image: UIImage(systemName: "plus"),
                style: .plain,
                target: context.coordinator, action: #selector(Coordinator.plusTapped)
            ),
            UIBarButtonItem(
                image: UIImage(systemName: "minus"),
                style: .plain,
                target: context.coordinator, action: #selector(Coordinator.minusTapped)
            ),
            UIBarButtonItem(
                image: UIImage(systemName: "multiply"),
                style: .plain,
                target: context.coordinator, action: #selector(Coordinator.timesTapped)
            ),
            UIBarButtonItem(
                image: UIImage(systemName: "divide"),
                style: .plain,
                target: context.coordinator, action: #selector(Coordinator.divideTapped)
            ),
    ...
    final class Coordinator: NSObject, UITextFieldDelegate {
        ...
        /// Running total and pending operator for quick in-field math.
        /// Left-to-right only, no operator precedence — this is a
        /// quick-entry aid, not a full calculator.
        ...
        func textFieldDidEndEditing(_ textField: UITextField) {
            // Losing focus (Done, tapping elsewhere, or Save) always leaves
            // a plain evaluated amount behind, never a dangling expression.
            finalizeExpression()
            applyDisplay(to: textField)
        }

        @objc func plusTapped() { handleOperator("+") }
        @objc func minusTapped() { handleOperator("-") }
        @objc func timesTapped() { handleOperator("×") }
        @objc func divideTapped() { handleOperator("÷") }

        private func handleOperator(_ op: Character) {
            guard let textField else { return }
            let hasTypedOperand = !integerDigits.isEmpty || hasDecimalPoint

            guard accumulatedValue != nil || hasTypedOperand else {
                // Nothing entered yet - an operator alone means nothing.
                return
            }
            ...
            if hasTypedOperand {
                ...
                if let acc = accumulatedValue, let pending = pendingOperator {
                    accumulatedValue = apply(pending, acc, operand)
                } else {
                    accumulatedValue = operand
                }
                resetOperand()
            }
            // If nothing was typed since the last operator, this just swaps
            // which operator will be applied next.
            pendingOperator = op
            applyDisplay(to: textField)
        }
        ...
        private func finalizeExpression() {
            ...
            let hasTypedOperand = !integerDigits.isEmpty || hasDecimalPoint
            let operand = hasTypedOperand ? currentOperandValue() : 0
            let result = apply(pending, acc, operand)
            accumulatedValue = nil
            pendingOperator = nil
            setOperand(from: result)
        }
        ...
        /// Loads a computed result back into the digit-entry state so it can
        /// keep being edited (backspace, another operator, etc.) like any
        /// typed amount.
        private func setOperand(from value: Double) {
            let rounded = max(0, (value * 100).rounded() / 100)
            let cents = Int((rounded * 100).rounded())
        ...
        private func handleBackspace() {
            if hasDecimalPoint {
                ...
                // Nothing left in this operand - backspace undoes the
                // pending operator and returns to editing the running
                // total directly.
                pendingOperator = nil
        ...
        /// Plain "12.50"-style text for just the operand currently being
        /// typed, independent of any running total before it.
        ...
        /// Full field text: the running total and pending operator (if any),
        /// followed by the operand currently being typed.
        private func computeDisplay() -> String {
            let operandDisplay = computeOperandDisplay()
            guard let pending = pendingOperator, let acc = accumulatedValue else {
                return operandDisplay
            }
            let accText = String(format: "%.2f", acc)
            if operandDisplay.isEmpty {
                return "\(accText) \(pending) "
            }
            return "\(accText) \(pending) \(operandDisplay)"
        }
